...
class Clientes extends CI_Controller{
    ...
    public function __construct() {
        ...
        if(!$cliente_id || !$this->ordem_model->get_by_id('clientes', array('cliente_id' => $cliente_id))){
            ...
            if($this->form_validation->run()){
                ...
                if($this->db->table_exists('contas_receber')){
                    ...
                    if($cliente_ativo == 0 && $this->ordem_model->get_by_id('contas_receber', array('conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0))){
                        $this->session->set_flashdata('info', 'Esse cliente não pode ser desativado, pois existe uma conta em aberto!');
                        redirect('clientes');
                    }
                }
     ...
     public function check_rg_ie($cliente_rg_ie){
        ...
        if($this->ordem_model->get_by_id('clientes', array('cliente_rg_ie' => $cliente_rg_ie, 'cliente_id !=' => $cliente_id))){
     ...
     public function valida_cpf($cpf) {
        ...
        if ($this->input->post('cliente_id')) {
            ...
            if ($this->ordem_model->get_by_id('clientes', array('cliente_id !=' => $cliente_id, 'cliente_cpf_cnpj' => $cpf))) {
     ...
     public function valida_cnpj($cnpj) {
        ...
        if ($this->input->post('cliente_id')) {
            ...
            if ($this->ordem_model->get_by_id('clientes', array('cliente_id !=' => $cliente_id, 'cliente_cpf_cnpj' => $cnpj))) {
